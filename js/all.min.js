...
var Flowtime = (function() {
	...
	var z = (function() {
		...
		function be(b3, b2, b4) {
			var b1 = bq;
			var b0 = b3 == !M;
			if (b2 == true && bs[bT][bq].length > 0 && bC[bT][bq] < bs[bT][bq].length - 1 && b0 !=
				true && b4 == false) {
				bn(bT, bq)
			} else {
				b1 = 0;
		...
		function a9(b0, b1) {
			if (bs[bT][bq].length > 0 && bC[bT][bq] < bs[bT][bq].length - 1 && b0 != true && b1 ==
				false) {
				bn(bT, bq)
			} else {
								...
								if (b4 == bT) {
									if (b2 > bq) {
										...
										if (b2 < bq) {
											...
											if (b2 == bq) {
												if (p == true && (D.section > z.getPageIndex()
														.section || D.page > z.getPageIndex().page)) {
													for (var b3 = 0; b3 < b0.length; b3++) {
														bn(b4, b2, b3)
													}
												} else {
												...
												if (p == false) {
													...
													if (D.section > z.getPageIndex().section || D.page >
														z.getPageIndex().page) {
	...
	function aZ(a8) {
		if (a8.length > 0) {
			...
			var ba = document.querySelector(aU + "[data-id=__" + e[0] + "]") || document.querySelector(aU +
				"[data-prog=__" + e[0] + "]");
			if (ba != null) {
				var a9 = null;
				if (e.length > 1) {
					a9 = ba.querySelector(aK + "[data-id=__" + e[1] + "]") || ba.querySelector(aK +
						"[data-prog=__" + e[1] + "]")
				}
				if (a9 == null) {
	...
	function d(e) {
		return (e.getAttribute("data-id") != null ? e.getAttribute("data-id").replace(/__/, "") : e
			.getAttribute("data-prog").replace(/__/, ""))
	}
	...
	function a3(a9) {
		...
		if (Brav1Toolbox.testCSS("transform")) {
			if (C) {
				J.style[Brav1Toolbox.getPrefixed("transform")] = "translateX(" + -a8 + "px) translateY(" + -ba +
					"px)"
			} else {
				J.style[Brav1Toolbox.getPrefixed("transform")] = "translateX(" + -a8 * 100 + "%) translateY(" +
					-ba * 100 + "%)"
			}
		} else {
			if (C) {
					...
					for (var bg = 0; bg < ba.length; bg++) {
						...
						if (bf != undefined) {
							for (var bc = 0; bc < bf.length; bc++) {
								...
								if (ao) {
									bh.style[Brav1Toolbox.getPrefixed("transform")] = "translateX(" + a8 +
										"px) translateY(" + e + "px)"
								} else {
									bh.style[Brav1Toolbox.getPrefixed("transform")] = "translateX(" + a8 +
										"%) translateY(" + e + "%)"
	...
	function af(e, a8) {
		let t = document.getElementById("lastTitle");
	...
	function R(ba) {
		if (ba) {
			...
			J.style[Brav1Toolbox.getPrefixed("transform")] = "translate(" + e + "%, " + bc + "%) scale(" + bb /
				100 + ", " + bb / 100 + ")"
		}
	}
	...
	function Q(a8) {
		if (a8) {
			...
			J.style[Brav1Toolbox.getPrefixed("transform")] = "translate(" + e + "%, " + bb + "%) scale(" + ba /
				100 + ", " + ba / 100 + ")"
		}
	}
	...
	function ay(ba) {
		...
		let page = z.getPageIndex().page;
		let section = z.getPageIndex().section;
		console.log(page,section,page==4 && section == 6)
		var aCount=0;
		var time = 2000;
		...
		let viewHeight = document.documentElement.clientHeight;//获取浏览器窗口可视区的高度
		let viewWidth = document.documentElement.clientWidth;//获取浏览器窗口可视区的高度
		...
		setTimeout(function() {    //添加一个定时器一秒钟后开始执行
			
				  let imgs = document.querySelectorAll('img');    // 获取所有img标签
				   //console.log(imgs);
				  for (let item of imgs) {                // 使用for-of遍历所有图片
					  // 显示图片出现在浏览器的位置
					  let r = item.getBoundingClientRect();    // 找到每一个图片在可视区上的位置
					  
					  if (r.bottom > -10 && r.bottom<viewHeight+20 && r.left>-10 && r.left<viewWidth+20 ) {    //给个if语句如果图片的底部大于0并且小于这个浏览器的可视区域 ，就执行以下操作
						 console.log(item);
						  item.src = item.getAttribute('data-src');    // 获取data-original的路径传给img
...
(function(h, o, g) {
	var p = function() {
		for (var b = /audio(.min)?.js.*/, a = document.getElementsByTagName("script"), c = 0, d = a.length; c <
			d; c++) {
	...
	g[h] = {
		...
		helpers: {
			...
			injectCss: function(b, a) {
				for (var c = "", d = document.getElementsByTagName("style"), e = a.replace(/\$1/g, b
						.settings.imageLocation), i = 0, f = d.length; i < f; i++) {
				...
				if (b.addEventListener) b.addEventListener(a, c, false);
					...
					if (!this.memoryLeaking) {
						window.attachEvent("onunload", function() {
							if (this.listeners)
								for (var d = 0, e = this.listeners.length; d < e; d++) g[h].events
	...
	g[o].prototype = {
		...
		loadProgress: function() {
			if (this.element.buffered != null && this.element.buffered.length) {
				...
				this.loadedPercent = this.element.buffered.end(this.element.buffered.length - 1) / this
					.duration;
		...
		play: function() {
			/(ipod|iphone|ipad)/i.test(navigator.userAgent) && this.element.readyState == 0 && this.init
				.apply(this);
